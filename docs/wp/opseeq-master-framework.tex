...
The source paper \texttt{agent-builder-framework.tex} (``A Bounded Graph-of-Thoughts Pipeline for Research-to-Rust Agent Creation under Local Mixture-of-Experts Inference'') now exists in \texttt{docs/wp/} and describes a bounded GoT pipeline for agent creation. Its content overlaps with the OODA orchestration pipeline (\S\ref{subsec:ooda}), the HPC-GoT controller (\S\ref{subsec:hpcgot}), and the \cellar{} runtime (\S\ref{subsec:cellar}). A future revision of this master document should integrate its novel contributions---particularly the research-to-Rust compilation pipeline---into Layers II--III and add it to the bibliography.
